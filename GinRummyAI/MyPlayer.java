...
public class MyPlayer implements GinRummyPlayer {
	...
	public boolean willDrawFaceUpCard(Card card) {
		...
		//		//check deadwoods AFTER adding the face up card
		//		int afterDeadwoods = 0;
		//		if(bestMeldSets.isEmpty()) {
		//			afterDeadwoods = GinRummyUtil.getDeadwoodPoints(cards);
		//			int highestDeadwoodCard = -1;
		//			for(Card c : cards) {
		//				int currPtn = GinRummyUtil.getDeadwoodPoints(c);
		//				if (currPtn > highestDeadwoodCard) {
		//					highestDeadwoodCard = currPtn;
		//				}
		//			}
		//			afterDeadwoods -= highestDeadwoodCard;
		//		}
		//		else {
		//			bestMelds = bestMeldSets.get(0);
		//			unmeldedCards = OurUtilities.cardsNotInMeld(bestMelds, newCards);
		//			
		//			if (!unmeldedCards.isEmpty()) {
		//				afterDeadwoods = GinRummyUtil.getDeadwoodPoints(unmeldedCards);
		//				int highestDeadwoodCard = 0;
		...
		//			
		//		}
		//		System.out.println("Deadwoods: BEFORE: " + deadwoods + " AFTER: "+ afterDeadwoods);
		//		if(afterDeadwoods <= deadwoods) {
		//			return true;
		//		}
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, ClassNotFoundException, FileNotFoundException, UnsupportedEncodingException {
		String[] playerNames = {"MyPlayer", "SimpleGinRummyPlayer2"};
